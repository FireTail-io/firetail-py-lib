...
def test_api_base_path_slash():
    ...
    assert api.blueprint.name == "base"
    assert api.blueprint.url_prefix == ""


def test_template():
    api1 = FlaskApi(TEST_FOLDER / "fixtures/simple/swagger.yaml", base_path="/api/v1.0", arguments={"title": "test"})
    assert api1.specification["info"]["title"] == "test"

    api2 = FlaskApi(
        TEST_FOLDER / "fixtures/simple/swagger.yaml", base_path="/api/v1.0", arguments={"title": "other test"}
    )
...
def test_invalid_operation_does_stop_application_to_setup():
    with pytest.raises(ImportError):
        FlaskApi(TEST_FOLDER / "fixtures/op_error_api/swagger.yaml", base_path="/api/v1.0", arguments={"title": "OK"})

    with pytest.raises(ValueError):
        FlaskApi(TEST_FOLDER / "fixtures/missing_op_id/swagger.yaml", base_path="/api/v1.0", arguments={"title": "OK"})
...
def test_invalid_operation_does_not_stop_application_in_debug_mode():
    api = FlaskApi(
        TEST_FOLDER / "fixtures/op_error_api/swagger.yaml", base_path="/api/v1.0", arguments={"title": "OK"}, debug=True
    )
...
def test_other_errors_stop_application_to_setup():
    ...
    with pytest.raises(InvalidSpecification):
        FlaskApi(TEST_FOLDER / "fixtures/bad_specs/swagger.yaml", base_path="/api/v1.0", arguments={"title": "OK"})
