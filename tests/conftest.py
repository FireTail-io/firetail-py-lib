...
def build_app_from_fixture(api_spec_folder, spec_file="openapi.yaml", **kwargs):
    ...
    cnx_app = App(__name__, port=5001, specification_dir=FIXTURES_FOLDER / api_spec_folder, debug=debug)
...
def snake_case_app(request):
    return build_app_from_fixture("snake_case", request.param, validate_responses=True, pythonic_params=True)
...
def strict_app(request):
    return build_app_from_fixture("simple", request.param, validate_responses=True, strict_validation=True)
...
def secure_endpoint_app(request):
    return build_app_from_fixture(
        "secure_endpoint", request.param, validate_responses=True, pass_context_arg_name="req_context"
    )
...
def secure_api_app(request):
    ...
    return build_app_from_fixture("secure_api", request.param, options=options, auth_all_paths=True)
