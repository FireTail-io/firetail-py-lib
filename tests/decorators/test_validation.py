...
def test_get_nullable_parameter():
    param = {"schema": {"type": "number", "nullable": True}, "required": True, "name": "foo"}
...
def test_get_valid_parameter_with_enum_array_header():
    value = "VALUE1,VALUE2"
    param = {
        "schema": {"type": "array", "items": {"type": "string", "enum": ["VALUE1", "VALUE2"]}},
...
def test_formdata_extra_parameter_strict():
    ...
    # Swagger 2
    validator = ParameterValidator([{"in": "formData", "name": "param"}], FlaskApi, strict_validation=True)
    errors = validator.validate_formdata_parameter_list(request)
    assert errors
