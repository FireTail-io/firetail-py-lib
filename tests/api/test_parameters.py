...
def test_formdata_bad_request(simple_app):
    ...
    assert resp.status_code == 400
    response = json.loads(resp.data.decode("utf-8", "replace"))
    assert response["detail"] in ["Missing formdata parameter 'formData'", "'formData' is a required property"]  # OAS3
...
def test_formdata_extra_param(simple_app):
    app_client = simple_app.app.test_client()
    resp = app_client.post("/v1.0/test-formData-param", data={"formData": "test", "extra_formData": "test"})
    assert resp.status_code == 200
...
def test_formdata_file_upload(simple_app):
    ...
    resp = app_client.post(
        "/v1.0/test-formData-file-upload", data={"formData": (BytesIO(b"file contents"), "filename.txt")}
    )
    assert resp.status_code == 200
    response = json.loads(resp.data.decode("utf-8", "replace"))
...
def test_args_kwargs(simple_app):
    ...
    if simple_app._spec_file == "openapi.yaml":
        body = {"foo": "a", "bar": "b"}
        resp = app_client.post(
            "/v1.0/body-params-as-kwargs", data=json.dumps(body), headers={"Content-Type": "application/json"}
        )
        assert resp.status_code == 200
...
def test_param_sanitization(simple_app):
    ...
    resp = app_client.post("/v1.0/param-sanitization?$query=queryString", data={"$form": "formString"})
    assert resp.status_code == 200
    assert json.loads(resp.data.decode("utf-8", "replace")) == {
        "query": "queryString",
        "form": "formString",
    ...
    resp = app_client.post(
        "/v1.0/body-sanitization", data=json.dumps(body), headers={"Content-Type": "application/json"}
    )
    assert resp.status_code == 200
...
def test_parameters_snake_case(snake_case_app):
    ...
    assert resp.status_code == 200
    resp = app_client.post(
        "/v1.0/test-post-query-shadow?id=123&class=header", headers=headers, data=json.dumps({"a": "test"})
    )
    assert resp.status_code == 200
