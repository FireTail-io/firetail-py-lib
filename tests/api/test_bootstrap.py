...
def test_app_with_relative_path(simple_api_spec_dir, spec):
    ...
    app = App(__name__, port=5001, specification_dir=".." / simple_api_spec_dir.relative_to(TEST_FOLDER), debug=True)
    app.add_api(spec)
...
def test_app_with_resolver(simple_api_spec_dir, spec):
    ...
    resolver = Resolver()
    app = App(
        __name__, port=5001, specification_dir=".." / simple_api_spec_dir.relative_to(TEST_FOLDER), resolver=resolver
    )
...
def test_app_with_different_uri_parser(simple_api_spec_dir):
    ...
    app_client = app.app.test_client()
    resp = app_client.get("/v1.0/test_array_csv_query_param?items=a,b,c&items=d,e,f")  # type: flask.Response
...
def test_swagger_ui_with_config(simple_api_spec_dir, spec):
    ...
    app = App(__name__, port=5001, specification_dir=simple_api_spec_dir, options=options, debug=True)
    app.add_api(spec)
    app_client = app.app.test_client()
    swagger_ui = app_client.get("/v1.0/ui/")  # type: flask.Response
    assert swagger_ui.status_code == 200
