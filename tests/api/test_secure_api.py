...
def test_security_over_nonexistent_endpoints(oauth_requests, secure_api_app):
    ...
    post_greeting = app_client.post(
        '/v1.0/greeting/rcaricio', data={}, headers=headers)  # type: flask.Response
    assert post_greeting.status_code == 200

    post_greeting = app_client.post(
        '/v1.0/greeting/rcaricio', data={})  # type: flask.Response
...
def test_security(oauth_requests, secure_endpoint_app):
    ...
    get_bye_no_auth = app_client.get(
        '/v1.0/byesecure/jsantos')  # type: flask.Response
    ...
    get_bye_no_auth_reponse = json.loads(
        get_bye_no_auth.data.decode('utf-8', 'replace'))  # type: dict
    ...
    get_bye_good_auth = app_client.get(
        '/v1.0/byesecure/jsantos', headers=headers)  # type: flask.Response
    ...
    get_bye_wrong_scope_reponse = json.loads(
        get_bye_wrong_scope.data.decode('utf-8', 'replace'))  # type: dict
    ...
    get_bye_bad_token_reponse = json.loads(
        get_bye_bad_token.data.decode('utf-8', 'replace'))  # type: dict
    ...
    response = app_client.get(
        '/v1.0/more-than-one-security-definition')  # type: flask.Response
    assert response.status_code == 401

    # also tests case-insensitivity
    headers = {"X-AUTH": "mykey"}
    response = app_client.get(
        '/v1.0/more-than-one-security-definition', headers=headers)  # type: flask.Response
    ...
    get_bye_from_flask = app_client.get(
        '/v1.0/byesecure-from-flask', headers=headers)  # type: flask.Response
...
def test_checking_that_client_token_has_all_necessary_scopes(
    ...
    response = app_client.get(
        '/v1.0/more-than-one-scope', headers=headers)  # type: flask.Response
