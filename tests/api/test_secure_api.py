...
def test_security(oauth_requests, secure_endpoint_app):
    ...
    headers = {"Authorization": "Bearer 100"}
    get_bye_from_firetail = app_client.get('/v1.0/byesecure-from-firetail', headers=headers)  # type: flask.Response
    assert get_bye_from_firetail.data == b'Goodbye test-user (Secure!)'

    headers = {"Authorization": "Bearer 100"}
    get_bye_from_firetail = app_client.get('/v1.0/byesecure-jwt/test-user', headers=headers)  # type: flask.Response
