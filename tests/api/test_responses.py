...
def test_empty_object_body(simple_app):
    ...
    resp = app_client.post(
        "/v1.0/test-empty-object-body", data=json.dumps({}), headers={"Content-Type": "application/json"}
    )
    assert resp.status_code == 200
    response = json.loads(resp.data.decode("utf-8", "replace"))
...
def test_post_wrong_content_type(simple_app):
    app_client = simple_app.app.test_client()
    resp = app_client.post(
        "/v1.0/post_wrong_content_type", content_type="application/xml", data=json.dumps({"some": "data"})
    )
    ...
    resp = app_client.post(
        "/v1.0/post_wrong_content_type", content_type="application/x-www-form-urlencoded", data="a=1&b=2"
    )
    assert resp.status_code == 415
    ...
    builder = EnvironBuilder(path="/v1.0/post_wrong_content_type", method="POST", data=json.dumps({"some": "data"}))
    try:
    ...
    resp = app_client.post("/v1.0/post_wrong_content_type", content_type="application/json", data="not a valid json")
...
def test_oneof(simple_openapi_app):
    ...
    post_greeting = app_client.post(
        "/v1.0/oneof_greeting", data=json.dumps({"name": 3}), content_type="application/json"
    )  # type: flask.Response
    assert post_greeting.status_code == 200
    assert post_greeting.content_type == "application/json"
    greeting_reponse = json.loads(post_greeting.data.decode("utf-8", "replace"))
    ...
    post_greeting = app_client.post(
        "/v1.0/oneof_greeting", data=json.dumps({"name": True}), content_type="application/json"
    )  # type: flask.Response
    assert post_greeting.status_code == 200
    assert post_greeting.content_type == "application/json"
    greeting_reponse = json.loads(post_greeting.data.decode("utf-8", "replace"))
    ...
    post_greeting = app_client.post(
        "/v1.0/oneof_greeting", data=json.dumps({"name": "jsantos"}), content_type="application/json"
    )  # type: flask.Response
