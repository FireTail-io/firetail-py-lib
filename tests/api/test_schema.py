...
def test_schema(schema_app):
    ...
    good_request = app_client.post(
        "/v1.0/test_schema", headers=headers, data=json.dumps({"image_version": "version"})
    )  # type: flask.Response
    ...
    good_request_extra = app_client.post(
        "/v1.0/test_schema", headers=headers, data=json.dumps({"image_version": "version", "extra": "stuff"})
    )  # type: flask.Response
...
def test_schema_in_query(schema_app):
    ...
    good_request = app_client.post(
        "/v1.0/test_schema_in_query", headers=headers, query_string={"image_version": "version", "not_required": "test"}
    )  # type: flask.Response
    assert good_request.status_code == 200
    good_request_response = json.loads(good_request.data.decode("utf-8", "replace"))  # type: dict
    assert good_request_response["image_version"] == "version"
...
def test_schema_map(schema_app):
    ...
    valid_object = {"foo": {"image_version": "string"}, "bar": {"image_version": "string"}}
...
def test_schema_int(schema_app):
    ...
    array_request = app_client.post("/v1.0/schema_int", headers=headers, data=json.dumps(42))  # type: flask.Response
    assert array_request.status_code == 200
    assert array_request.content_type == "application/json"
    array_response = json.loads(array_request.data.decode("utf-8", "replace"))  # type: list
