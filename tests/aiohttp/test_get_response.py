...
async def test_get_response_from_firetail_response(api):
    response = await api.get_response(
        FiretailResponse(status_code=201, mimetype="text/plain", body="foo", headers={"X-header": "value"})
    )
    assert isinstance(response, web.Response)
    assert response.status == 201
    assert response.body == b"foo"
...
async def test_get_firetail_response_from_firetail_response(api):
    response = api.get_firetail_response(
        FiretailResponse(status_code=201, content_type="text/plain", body="foo", headers={"X-header": "value"})
    )
    assert isinstance(response, FiretailResponse)
    assert response.status_code == 201
    assert response.body == b"foo"
