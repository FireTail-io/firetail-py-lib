...
async def test_get_response_from_firetail_response(api):
    response = await api.get_response(FiretailResponse(status_code=201, mimetype='text/plain', body='foo', headers={'X-header': 'value'}))
    assert isinstance(response, web.Response)
    assert response.status == 201
    assert response.body == b'foo'
    assert response.content_type == 'text/plain'
...
async def test_get_firetail_response_from_aiohttp_response(api):
    response = api.get_firetail_response(web.Response(text='foo', status=201, headers={'X-header': 'value'}))
    assert isinstance(response, FiretailResponse)
...
async def test_get_firetail_response_from_firetail_response(api):
    response = api.get_firetail_response(FiretailResponse(status_code=201, content_type='text/plain', body='foo', headers={'X-header': 'value'}))
    assert isinstance(response, FiretailResponse)
    assert response.status_code == 201
    assert response.body == b'foo'
    assert response.content_type == 'text/plain'
...
async def test_get_firetail_response_from_tuple(api):
    response = api.get_firetail_response(('foo', 201, {'X-header': 'value'}))
    assert isinstance(response, FiretailResponse)
    assert response.status_code == 201
    assert response.body == b'foo'
    assert response.content_type == 'text/plain'
...
async def test_get_firetail_response_from_aiohttp_stream_response(api):
    response = api.get_firetail_response(web.StreamResponse(status=201, headers={'X-header': 'value'}))
    assert isinstance(response, FiretailResponse)
    assert response.status_code == 201
