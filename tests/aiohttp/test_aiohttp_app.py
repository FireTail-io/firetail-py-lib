...
def web_run_app_mock(monkeypatch):
    ...
    monkeypatch.setattr("firetail.apps.aiohttp_app.web.run_app", mock_)
    return mock_


@pytest.fixture
def sys_modules_mock(monkeypatch):
    monkeypatch.setattr("firetail.apps.aiohttp_app.sys.modules", {})
...
def test_app_run(web_run_app_mock, aiohttp_api_spec_dir):
    ...
    assert web_run_app_mock.call_args_list == [mock.call(app.app, port=5001, host="0.0.0.0", access_log=logger)]


def test_app_run_new_port(web_run_app_mock, aiohttp_api_spec_dir):
    app = AioHttpApp(__name__, port=5001, specification_dir=aiohttp_api_spec_dir, debug=True)
    app.run(port=5002)
    assert web_run_app_mock.call_args_list == [mock.call(app.app, port=5002, host="0.0.0.0", access_log=None)]
...
def test_app_run_default_port(web_run_app_mock, aiohttp_api_spec_dir):
    ...
    app.run()
    assert web_run_app_mock.call_args_list == [mock.call(app.app, port=5000, host="0.0.0.0", access_log=None)]
...
def test_app_run_debug(web_run_app_mock, aiohttp_api_spec_dir):
    ...
    app.run(debug=True)
    assert web_run_app_mock.call_args_list == [mock.call(app.app, port=5001, host="0.0.0.0", access_log=None)]
...
def test_app_get_root_path_not_in_sys_modules(sys_modules_mock, aiohttp_api_spec_dir):
    app = AioHttpApp("firetail", port=5001, specification_dir=aiohttp_api_spec_dir)
    root_path = app.get_root_path()
    assert str(root_path).endswith(os.sep + "firetail") == True
...
def test_app_with_empty_base_path_error(aiohttp_api_spec_dir):
    ...
    with pytest.raises(FiretailException) as exc_info:
...
def test_app_with_empty_base_path_and_only_one_api(aiohttp_api_spec_dir):
    spec_dir = ".." / aiohttp_api_spec_dir.relative_to(TEST_FOLDER)
    app = AioHttpApp(__name__, port=5001, specification_dir=spec_dir, debug=True, only_one_api=True)
...
def test_app_add_two_apis_error_with_only_one_api(aiohttp_api_spec_dir):
    ...
    assert exc_info.value.args == (
        "an api was already added, " "create a new app with 'only_one_api=False' " "to add more than one api",
    )
