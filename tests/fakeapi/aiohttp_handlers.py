...
async def aiohttp_users_post(user):
    if "name" not in user:
        return FiretailResponse(body={"error": "name is undefined"}, status_code=400, content_type="application/json")
    user["id"] = len(USERS) + 1
    USERS.append(user)
...
async def get_datetime():
    return FiretailResponse(body={"value": datetime.datetime(2000, 1, 2, 3, 4, 5, 6)})


async def get_date():
    return FiretailResponse(body={"value": datetime.date(2000, 1, 2)})


async def get_uuid():
    return FiretailResponse(body={"value": uuid.UUID(hex="e7ff66d0-3ec2-4c4e-bed0-6e4723c24c51")})


async def aiohttp_multipart_single_file(myfile):
    return aiohttp.web.json_response(
        data={"fileName": myfile.filename, "myfile_content": myfile.file.read().decode("utf8")},
    )


async def aiohttp_multipart_many_files(myfiles):
    return aiohttp.web.json_response(
        data={"files_count": len(myfiles), "myfiles_content": [f.file.read().decode("utf8") for f in myfiles]},
    )
