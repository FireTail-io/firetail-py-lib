...
def mock_get_function_from_name(monkeypatch):
    ...
    monkeypatch.setattr("firetail.cli.firetail.utils.get_function_from_name", get_function_from_name)
...
def test_run_spec_with_host(mock_app_run, spec_file):
    ...
    app_instance = mock_app_run()
    app_instance.run.assert_called_with(port=default_port, host="custom.host", server="flask", debug=False)
...
def test_run_in_debug_mode(mock_app_run, expected_arguments, spec_file, monkeypatch):
    ...
    monkeypatch.setattr("firetail.cli.logging.basicConfig", logging_config)
...
def test_run_using_option_base_path(mock_app_run, expected_arguments, spec_file):
    ...
    expected_arguments = dict(base_path="/foo", resolver_error=None, validate_responses=False, strict_validation=False)
...
def test_run_with_wsgi_server_and_server_opts(mock_app_run, spec_file):
    ...
    result = runner.invoke(main, ["run", spec_file, "-w", "flask", "-s", "flask"], catch_exceptions=False)
...
def test_run_with_incompatible_server_and_framework(mock_app_run, spec_file):
    ...
    result = runner.invoke(main, ["run", spec_file, "-s", "flask", "-f", "aiohttp"], catch_exceptions=False)
