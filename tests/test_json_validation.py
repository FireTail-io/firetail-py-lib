...
def test_validator_map(json_validation_spec_dir, spec):
    ...
    MinLengthRequestValidator = extend(
        Draft4RequestValidator, {'type': validate_type})
    ...
    res = app_client.post('/v1.0/minlength', data=json.dumps(
        {'foo': 'bar'}), content_type='application/json')  # type: flask.Response
    assert res.status_code == 200

    res = app_client.post('/v1.0/minlength', data=json.dumps(
        {'foo': ''}), content_type='application/json')  # type: flask.Response
...
def test_readonly(json_validation_spec_dir, spec):
    ...
    res = app_client.post('/v1.0/user', data=json.dumps(
        {'name': 'max', 'password': '1234'}), content_type='application/json')  # type: flask.Response
    assert res.status_code == 200
    assert json.loads(res.data.decode()).get('user_id') == 8

    res = app_client.post('/v1.0/user', data=json.dumps(
        {'user_id': 9, 'name': 'max'}), content_type='application/json')  # type: flask.Response
    assert res.status_code == 400
...
def test_multipart_form_json(json_validation_spec_dir, spec):
    ...
    res = app_client.post('/v1.0/multipart_form_json', data={'x': json.dumps(
        {"name": "joe", "age": 20})}, content_type='multipart/form-data')
    assert res.status_code == 200
